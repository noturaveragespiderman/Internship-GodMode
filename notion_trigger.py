...
def get_new_companies(last_processed_id: set):
    ...
    for page in results:
        ...
        def get_text(prop):
            ...
            # Safely combine chunks if reading a long text from Notion
            return "".join(item["text"]["content"] for item in items) if items else ""
...
def update_notion_row(page_id: str, data: dict):
    ...
    for notion_field, (field_type, data_key) in field_map.items():
        ...
        if field_type == "rich_text":
            # Slice text into 2000-character chunks to bypass Notion's API limits
            chunks = [value_str[i:i+2000] for i in range(0, len(value_str), 2000)]
            properties[notion_field] = {"rich_text": [{"text": {"content": chunk}} for chunk in chunks]}
        
        elif field_type == "url":
            properties[notion_field] = {"url": value_str}
        elif field_type == "email":
            properties[notion_field] = {"email": value_str}

    if "date_sent" in data or "email_sent_date" in data:
        if data.get("date_sent") or data.get("email_sent_date"):
